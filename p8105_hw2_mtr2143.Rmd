...
There are `r nrow(trash_data)` observations and `r ncol(trash_data)` columns in the Mr. Trash Wheel dataset and `r nrow(precip_18_19_data)` observations and `r ncol(precip_18_19_data)` columns in the combined precipitation dataset. The median number of sports balls consumed by Mr. Trash Wheel in 2019 is `r trash_data %>% filter(year == 2019) %>% summarise(median(sports_balls))`. Key variables communicate type of trash Mr. Trash Wheel consumes, such as:
...
Meanwhile, the precipitation data tell us about the total rainfall (inches) per month for the years `r min(pull(precip_18_19_data))` and `r max(pull(precip_18_19_data))`.  The total inches of rainfall in 2018 was `r sum(pull(precip_2018_data, total))`.
